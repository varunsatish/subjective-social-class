...
r2_trends_relabelled <- r2_trends %>%
  mutate(explanatory_variable = case_when(
    explanatory_variable == "all" ~ "All below \n included",
...
ggplot(r2_trends_relabelled, aes(x = year, y = r_squared, color = outcome_variable, linetype=explanatory_variable, shape=explanatory_variable)) +
  ...
  guides(
    ...
    shape = guide_legend(override.aes = list(color = "black"), nrow=1),
    linetype = guide_legend(override.aes = list(color = "black"), nrow=1)
    ) + 
...
ggsave(paste0(figure_save_path, "/r2_trends_over_time.pdf"), width = 8, height = 8)
